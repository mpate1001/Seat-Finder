...
describe('runDetectionPipeline', () => {
  it('emits progress in order preparing → scanning → cropping → ocr → done', async () => {
    ...
    const stages: string[] = [];
    await runDetectionPipeline(fakeBitmap(1000, 800), 'plan.png', (p) => {
      stages.push(p.stage);
    });
  ...
  it("sets status='ok' when confidence>=60 and digits parsed", async () => {
    ...
    const pins = await runDetectionPipeline(fakeBitmap(1000, 800), 'plan.png', () => {});
  ...
  it('guards getImageData against negative x/y when a circle sits near the canvas edge', async () => {
    ...
    await runDetectionPipeline(fakeBitmap(1000, 800), 'plan.png', () => {});
